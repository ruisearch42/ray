...
class _DAGNodeOperation:
    ...
    def next_operation(self):
        """
        The expected next operation of the same task to perform after this operation.

        If the current operation is READ, this function returns COMPUTE with the same
        `exec_task_idx` and method name. If the current operation is COMPUTE, this
        function returns WRITE with the same `exec_task_idx` and method name.
        Calling this function on a WRITE operation will raise a ValueError.
        """
...
def _visualize_execution_schedule(
    ...
    Args:
        ...
        actor_to_optimized_nodes: A dictionary that maps an actor handle to the
    ...
    dot.render("adag_schedule", format="png", view=True)
